...
e.g. a 2-dimensional, 2 by 3 array (or rank 2 array with extent 2 3) 
% (BLM) a "+1"(?) here for taking this opportunity to introduce "your" terminology (rank, extent).
of integers would be implemented as an array of 2 arrays, with each of these containing 3 integers.
...
% (BLM) the mention of inherent data parallelism here seems like not quite what's wanted in context??  Maybe you want two sentences??  one about how lack of functional-language abstractions means you can't easily extend to higher dimensions, and another about how having to use for loops hides the inherent data parallelism?  
...
% (BLM) Maybe the previous sentence would work better as an itemized list -- makes it clearer what the individual elements are?  I don't feel strongly about it though.
% (BLM) ALSO -- I think you need a footnote or something about "tacit" so readers understand that in the J world it has a specific technical meaning.  A brief footnote giving a short definition plus a reference to J documentation should do it, I think.
...
% (BLM) Here too I wonder whether an itemized list would work well?
...
% (BLM) You might say here something about how we realized that it might not be very meaningful to compare raw performance of C versus parallel-J but that we thought it *would* be meaningful to explore how parallel-J performance scales relative to number of threads -- as a way of finding out whether the implementation actual does effectively exploit potential concurrency.
...
% (BLM) The above is new to me, but -- yes, that *IS* what you're doing, in some sense, and I like making that explicit!
...
% (BLM) "in the time available"?  otherwise it seems like it can't be done at all.
