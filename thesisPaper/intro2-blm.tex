...
% (BLM) Somehow that "To the [first/second] point seems awkward. 
% What if instead of the numbered list above you have two paragraphs:
% "First, the implementation ...." followed by the current "To the first
% point" paragraph, then "Second, in many use cases ..." etc.  ?
...
% (BLM) I say it wouldn't hurt to define data rank ....
% Oh, you have that in a "comment" section below.  I thought I'd read
% it in a previous draft!  Maybe you could put it as a footnote here?
...
\section{Design Plan}
% (BLM) I don't like this section title but can't think of a better one.
% "Goals of this work"?  "Proposed solution"????
% (BLM) Somewhere in this section I think you could/should mention the
% work you did on parsing J.  How about something along the following
% lines:
%
% "We therefore propose a parallel implementation of J 
% that exploits the potential concurrency found in J's
% [whatever] and adds a new operator that [something]."
% The "in Scala" part ....  For a compiler (as opposed to an
% interpreter) it would be useful to distinguish between the
% target of the compilation (Scala here) and the language doing
% the compiling (also Scala here).  I don't know if that makes
% sense for an interpreter but if so it's worth doing.
% 
% Then mention that a full implementation was not feasible in
% the available time, but as a proof of concept you present
% a partial implementation, and mention that you did some work
% on the parsing part of the problem too.  Since you actually
% made nontrivial progress on that part I vote to include a
% description of what you did and perhaps even (some) code.
% Maybe that could go in an appendix.
% 
...
% (BLM) "Chapter" not "Section" -- oops.  Also maybe this chapter
% could be explicitly described as conclusions and thoughts about
% future work.
% And please do replace those hardcoded numbers ....
% Put a \label command after \chapter in each chapter, and use \ref
% here.
\end{itemize}

% (BLM) probably these should be referenced somewhere sometime --
% a "TODO"?
\nocite{rankanduni}
\nocite{dph}
